...
import data.process.RatingStatus.RatingStatus
import data.process.ResolvingStatus.ResolvingStatus
...
class DataExtractor {
    ...
    def extractRatingCallsFromExcel(
        ...
        ).map(x => if (null != x && "" != x) x else "unknown")
        ...
        val callsWithCategories = calls.zip(categories).filter { x =>
            ...
            stopWords.foreach(word => acceptableCall = acceptableCall && !x._1.toLowerCase.contains(word.toLowerCase))
        ...
        resultFromFillToFill.foreach { fullInfo =>
            if (
                RatingStatus.Unknown == fullInfo.ratingStatus && ({
                    ...
                    var res = false
                    acceptableWords.foreach { word => res = res || fullInfo.call.toLowerCase.contains(word.toLowerCase) }
                    res
                })
            ) {
                fullInfo.ratingStatus = status
